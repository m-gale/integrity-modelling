...
obs_path=scrap_dir+'pts_updated_500m_v11_1kmspacing_20volexp_sampled_v1.csv'

df=pd.read_csv(obs_path)
print(df)
df['geometry'] = df['geometry'].apply(wkt.loads)
pts = gpd.GeoDataFrame(df, geometry='geometry')
pts.set_crs(epsg=3577, inplace=True)
...
"""
Load residuals
For now absolute departure in AGB from reference to observed
"""

with rio.open(res_dir) as src:
...
#exclude the lowest condition class
...
for pass_no in pass_nos:
    ...
    for i in cuq:
        pts_updated_i=pts_updated[pts_updated['class_pca']==i]['source']
        try:
            if (~np.isnan(i)) & (sum(pts_updated_i.str.contains('residuals'))<20):
                print('')
                print('Pass '+str(pass_no))
                print(f"Processing class {i}")
                
                existing_samples = sum(pts_updated['class_pca'] == i)
                #determine residuals within cluster
                init_locs = (cluster_raster == i) & (ref_ras == 1) & (gdm_raster_mask) & (~np.isnan(res_raster))
                reses=res_raster[init_locs]
                if len(reses)>10:
                    p90=np.nanpercentile(reses, 95)
                    p10=np.nanpercentile(reses, 5)
                    ref_locs = np.zeros_like(res_raster, dtype=bool)
                    if (p90>0) & (p10<0):
                        ref_locs=(cluster_raster == i) & (ref_ras == 1) & (gdm_raster_mask) & ((res_raster>p90) | (res_raster<p10))
                    if (p90>0) & (p10>0):
                        ref_locs=(cluster_raster == i) & (ref_ras == 1) & (gdm_raster_mask) & (res_raster>p90)
                    if (p90<0) & (p10<0):
                        ref_locs=(cluster_raster == i) & (ref_ras == 1) & (gdm_raster_mask) & (res_raster<p10)
                ...
                if len(ref_rows) > 0:
                    ...
                    pts_coords = [(geom.x, geom.y) for geom in pts_updated[(pts_updated['class_pca']==i) & 
                                                                           (pts_updated['source'].str.contains('residuals', na=False))].geometry]
                            ...
                            if outside_points.shape[0] > 0:
                                ...
                                if len(new_sites)>0:
                                    ...
                                    if len(new_sites_filt)>0:
                                        ...
                                        new_df['source'] = '(residuals)_'+ref_fn.split('\\')[-1]
                                        new_df['class_pca'] = i
...
pts_updated[['class_pca', 'source', 'geometry']].to_file(scrap_dir+'pts_updated_500m_v11_1kmspacing_20volexp_residuals.shp')
