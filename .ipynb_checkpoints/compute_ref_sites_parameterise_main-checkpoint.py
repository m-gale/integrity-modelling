...
if os.path.isfile(out_dir+'pca_fit_'+str(n_comp)+'comp.joblib'):
    pca=joblib.load(out_dir+'pca_fit_'+str(n_comp)+'comp.joblib')
else:   
    ...
    pca = PCA(n_components=min(n_bands, n_comp))
    pca.fit(gdm_sample)  # Learn PCA components from sample
        
    gdm_pcs_valid = pca.transform(gdm_valid)
    n_pixels = gdm_2d.shape[0]
    pc_full = np.full((n_pixels, n_comp), np.nan)
    pc_full[mask, :] = gdm_pcs_valid
    
    del gdm_valid, gdm_sample, mask, gdm_2d
    del gdm_pcs_valid

    pc_stack = pc_full.reshape((n_rows, n_cols, n_comp)).transpose((2, 0, 1))  # shape: (3, rows, cols)
    
    with rio.open(
        out_dir+"gdm_pca_"+str(n_comp)+"comp.tif", "w",
        driver="GTiff",
        height=n_rows,
        width=n_cols,
        count=3,
    ...
    joblib.dump(pca, out_dir+'pca_fit_'+str(n_comp)+'comp.joblib')
    del pc_stack
...
#those intersecting NSW
sp_classes=[     1,           5,          12,          13,          16,
                19,          20,          22,          27,          30,
                36,          51,          54,          55,          66,
                68,          74,          76,          80,          84,
                85,          96,         102,         103,         110,
...
for sfactor in sfactors:
    for min_distance in [2000]:
        for cov_threshold in [0.8, 0.9, 0.95]:
            print('S-Factor: '+str(sfactor))
            print('Min-distance: '+str(min_distance))
            print('Cov-threshold: '+str(cov_threshold))
            ...
            diststr=str(distpa)
            n_compstr=str(n_comp)
            out_fn=out_dir+'pts_updated_250m_'+vno+'_'+mindi+'km_'+sfac+'volexp_'+diststr+'distp_'+cthr+'cov_'+n_compstr+'pca_95out_NSW-only.shp'
